...
// Compile with: 
// g++ -std=c++17 sched_energy_5.cpp -o sched_energy_5 -I./ -I/usr/local/include   -L/usr/local/lib -Wl,-rpath,/usr/local/lib   -lboost_coroutine -lboost_context -lboost_system   -libverbs -lmemcached -lpthread
...
void print_worker(Scheduler& sched, int tid, int coroid) {
    auto* source = new CoroCall([tid, coroid](CoroYield& yield) {
        Scheduler* current_sched = yield.get(); // This is correct, as the first call to operator()() will return the value passed
        std::cout << "[Coroutine " << tid << "-" << coroid << "] started on thread " << gettid() << "\n";
        Request r;
        while (true) {
            if (!current_sched->rx_queue.try_pop(r)) {
                current_sched = yield().get(); // The `get()` is still necessary here to retrieve the return value from the generator
                continue;
            }
            process_request_on_worker(r, tid, coroid);
            current_sched = yield().get();
        }
    });
}
